...
const sections = [
  ...
  {
    title: "2. Eligibility",
    body: `The Platform is exclusively available to verified residents of Barangay Pagatpatan, Cagayan de Oro City. You must be at least 13 years of age. Accounts are subject to identity verification by a barangay administrator. Providing false information during registration may result in permanent suspension.`,
  },
  {
    title: "3. Account Registration & Verification",
    body: `You are responsible for maintaining the confidentiality of your password and account. After registering, you must verify your email to gain access. To unlock full features like posting and commenting, you must complete identity verification by submitting a valid government-issued ID and a selfie. Your identity will be reviewed and approved by a barangay administrator. BarangayPGT is not liable for losses resulting from unauthorized account access.`,
  },
  {
  ...
  {
    title: "7. Privacy",
    body: `We collect personal information including your name, email, phone number, and address for the purpose of account verification and community management. Valid ID and selfie images submitted for verification are stored securely and accessible only to authorized administrators. This information is shared only with barangay officials as required for governance purposes. We do not sell your data to third parties. By using the Platform, you consent to receiving SMS notifications regarding your account status and barangay updates.`,
  },
  {
...
export default function TermsPage() {
  ...
  useEffect(() => {
    // Check if user is logged in by calling the profile API
    fetch("/api/profile?action=me")
      .then((r) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <nav className="sticky top-0 z-50 flex items-center justify-between border-b border-slate-100 bg-white/95 px-5 py-3 backdrop-blur-sm">
        <Link href={isLoggedIn ? "/feed" : "/"} className="flex items-center gap-2.5">
        ...
        {isLoggedIn && (
          <Link href="/feed" className="rounded-xl bg-blue-600 px-4 py-2 text-[12px] font-bold text-white hover:bg-blue-700">
            Back to Feed
          </Link>
        )}
      </nav>
      ...
      <div className="mx-auto max-w-2xl px-5 py-10 pb-20">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-[13px] text-slate-400">Effective Date: January 1, 2026 · Last updated: March 26, 2026</p>
        ...
        <div className="mb-8 rounded-2xl bg-blue-50 p-5 ring-1 ring-blue-100">
          ...
          <ul className="space-y-1 text-[13px] text-blue-700">
            <li>✅ Only Barangay Pagatpatan residents may register</li>
            <li>✅ Verify your identity (ID + selfie) to unlock full features</li>
