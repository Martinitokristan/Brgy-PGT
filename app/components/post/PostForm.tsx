...
import { X, Loader2, Send, ImageIcon, Pencil } from "lucide-react";
...
export function PostForm({ 
  ...
  const { title, description, purpose, urgency, selectedImage, imagePreview, isSubmitting, uploadProgress } = formState;
        ...
        <form onSubmit={async (e) => {
          ...
          <div className="px-5 py-4 space-y-3">
            <Input
              ...
              className="border-0 bg-transparent text-[18px] font-bold text-slate-900 focus:outline-none placeholder:text-slate-300"
            />
            <Textarea
              ...
              rows={4}
              className="resize-none border-0 bg-transparent text-[15px] text-slate-600 focus:outline-none placeholder:text-slate-300 leading-relaxed"
            />
            {imagePreview && (
          ...
          <div className="flex items-center justify-between gap-2 border-t border-slate-100 px-5 py-4">
            <div className="flex items-center gap-1">
              {!isEditMode && (
                <>
                  <input type="file" ref={fileInputRef} onChange={onImageSelect} accept="image/*" className="hidden" />
                  <Button type="button" onClick={() => fileInputRef.current?.click()} variant="ghost" size="sm"
                    className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:bg-slate-100 hover:text-blue-600">
